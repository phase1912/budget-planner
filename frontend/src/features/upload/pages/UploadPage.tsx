      ...
      <Stack className="gap-6">
        <div className="flex items-center">
          <span className="inline-flex items-center gap-2.5">
            <span className="inline-flex items-center justify-center w-7 h-7 border border-transparent rounded-pill bg-primary text-primary-foreground text-[13px] font-bold">1</span>
          ...
          <span className="inline-flex items-center gap-2.5">
            <span className="inline-flex items-center justify-center w-7 h-7 border border-border rounded-pill bg-background text-border-strong text-[13px] font-bold">2</span>
            <span className="text-sm font-semibold text-border-strong">What we read</span>
          </span>
          <span className="grow h-[2px] rounded-[1px] bg-border mx-4"></span>
          <span className="inline-flex items-center gap-2.5">
            <span className="inline-flex items-center justify-center w-7 h-7 border border-border rounded-pill bg-background text-border-strong text-[13px] font-bold">3</span>
        ...
        <div className="inline-flex gap-1 border border-border rounded-control bg-muted p-1 self-start" role="tablist">
          <button
            ...
            onClick={() => { uploadStore.setMode("single"); }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1Z"/><path d="M16 8H8"/><path d="M16 12H8"/></svg>
            One receipt
          </button>
          <button
            className={`inline-flex items-center gap-2 border-none rounded-[9px] px-4 py-[9px] text-sm cursor-pointer transition-colors ${
            ...
            onClick={() => { uploadStore.setMode("multiple"); }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 17 22 12"/></svg>
            Several receipts
          </button>
        </div>
        ...
        <Stack className="gap-4">
          {uploadStore.mode === "single" && uploadStore.files.length > 0 && (
            <Card variant="surface" className={`p-5 ${uploadStore.isOverLimit ? "border-tone-error-border" : ""}`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2.5">
                  <span className={`inline-flex items-center justify-center w-[26px] h-[26px] rounded-pill text-xs font-bold ${uploadStore.isOverLimit ? "bg-tone-error-bg text-tone-error-text" : "bg-tone-primary-bg text-tone-primary-text"}`}>1</span>
              ...
              <div className="flex items-center gap-2.5 mb-4 flex-wrap">
                {uploadStore.files.map((_, i) => (
                  <span key={i} className="relative inline-flex items-center justify-center border border-border rounded-chip bg-muted w-[72px] h-[88px] overflow-hidden">
                    <span className="absolute bottom-1 right-1 text-[10px] text-muted-foreground">{i + 1}</span>
                    <button 
                      onClick={() => { uploadStore.removeFile(i); }} 
              ...
              {uploadStore.isOverLimit && (
                <div className="flex items-start gap-2.5 rounded-card p-3 mb-4 bg-tone-error-bg text-tone-error-text">
                  <svg className="mt-[2px] text-tone-error-text shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>
              ...
              <Stack className="gap-[5px]">
                <div className="flex items-baseline justify-between">
                  <span className={`text-xs ${isOverCount ? "font-semibold text-error" : "text-muted-foreground"}`}>
                    {uploadStore.files.length} of 10 photos
                  </span>
                  <span className={`text-xs ${isOverSize ? "font-semibold text-error" : "text-muted-foreground"}`}>
                    {uploadStore.totalSizeMB} of 50&nbsp;MB
                  </span>
                </div>
                <span className="block h-[4px] rounded-[2px] bg-border overflow-hidden">
                  <span className={`block h-full rounded-[2px] ${uploadStore.isOverLimit ? "bg-error" : "bg-primary"}`} style={{ width: `${String(fillPercentage)}%` }}></span>
                </span>
              </Stack>
            </Card>
          )}
            ...
            <div className="card card--flush panel mt-4 card--attention">
              ...
              <div className="panel__body">
                <div className="note note--error items-start w-full">
                  <svg className="note__icon mt-0.5" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>
                  <div className="flex flex-col gap-1">
                    <span className="text-[13px] font-bold">Upload failed</span>
                    <span className="text-xs leading-relaxed">
                      {uploadStore.errorDetails}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {uploadStore.files.length > 0 && uploadStore.mode === "single" && (
          <div className="flex items-center justify-between gap-4 border-t border-border pt-5">
            <span className="text-[13px] text-muted-foreground">
              1 receipt &middot; {uploadStore.files.length} photos &middot; {uploadStore.totalSizeMB}&nbsp;MB total
            </span>
